...
def test_success(client):
    ...
    data = {
        ...
        "questions": [
            {
                "id": question["id"],
                "answers": [{"id": option["id"], "free_text": "TXT"}],
            }
        ],
    }
    ...
    ride_question = hlp.db.fetchall(
        "SELECT id, ride_survey_id, question_id FROM RideQuestions"
    )[0]
    ...
    ride_answers = hlp.db.fetchall(
        "SELECT id, ride_question_id, option_id, free_text FROM RideAnswers"
    )
